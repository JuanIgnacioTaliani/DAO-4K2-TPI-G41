...
from ..seed_extended import generate_extended_seed
...
def seed_database(
    ...
    mode: str = Query("extended", regex="^(basic|extended)$", description="Modo de seed: basic o extended"),
):
    """
    ...
    try:
        ...
        if mode == "basic":
            ...
            # Extended dataset
            clientes, empleados, categorias, estados, vehiculos, alquileres, multas, mantenimientos = generate_extended_seed()
            db.bulk_save_objects(clientes)
            db.bulk_save_objects(empleados)
            db.bulk_save_objects(categorias)
            db.bulk_save_objects(estados)
        ...
        return {
            "message": f"Base de datos poblada exitosamente (modo {mode})",
            "success": True,
